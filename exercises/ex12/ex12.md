...
Answer : There is definitely proof that the threads are running concurrently 
and that synchronization errors are happening - the counter variable definitely
does not increment in uniform, linearly spaced values.
...
Yes, there are some errors happening.
...
It looks like the synchronization results in it requirng about double the 
overhead for an individual computation. 
